...
interface RuleFormData {
  ...
  valueMin: string;
  valueMax: string;
  // ROAS specific (Tonic + TikTok calculation)
...
// TikTok uses AD_GROUP instead of AD_SET
const LEVELS = [
  { value: 'CAMPAIGN', label: 'Campana' },
  { value: 'AD_GROUP', label: 'Ad Group (Grupo de anuncios)' },
...
export default function TikTokRuleForm({ initialData, ruleId, mode }: TikTokRuleFormProps) {
  ...
  const [formData, setFormData] = useState<RuleFormData>({
    ...
    tiktokAccountId: initialData?.tiktokAccountId || '',
  ...
  // Fetch campaigns when TikTok account changes
  useEffect(() => {
    if (formData.tiktokAccountId) {
      fetchCampaigns(formData.tiktokAccountId);
    }
  }, [formData.tiktokAccountId]);

  const fetchAccounts = async () => {
    try {
      // Fetch TikTok accounts
      const tiktokResponse = await fetch('/api/accounts?type=TIKTOK');
      const tiktokData = await tiktokResponse.json();

      const tiktokAccountsList = tiktokData.data?.tiktok || tiktokData.data || [];

      if (tiktokData.success && Array.isArray(tiktokAccountsList)) {
        setTiktokAccounts(tiktokAccountsList);
        if (!initialData?.tiktokAccountId && tiktokAccountsList.length > 0) {
          setFormData(prev => ({ ...prev, tiktokAccountId: tiktokAccountsList[0].id }));
        }
      } else {
        console.error('Error fetching TikTok accounts:', tiktokData.error || 'Invalid response');
        setTiktokAccounts([]);
      }
  ...
  const fetchCampaigns = async (accountId: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/accounts/${accountId}/tiktok-campaigns`);
      ...
      if (data.success && Array.isArray(data.data)) {
        ...
        setCampaigns([]);
        if (data.error) {
          console.warn('Error fetching TikTok campaigns:', data.error);
        }
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!formData.tiktokAccountId) {
        throw new Error('Selecciona una cuenta TikTok');
      }
      ...
      const payload = {
        ...
        tiktokAccountId: formData.tiktokAccountId,
      ...
      const url = mode === 'create' ? '/api/tiktok-rules' : `/api/tiktok-rules/${ruleId}`;
  ...
  const handleCalculateRoas = async () => {
    if (!formData.tiktokAccountId) {
      setError('Selecciona una cuenta TikTok para calcular ROAS');
      return;
    }
    if (!formData.tonicAccountId) {
    ...
    try {
      const response = await fetch('/api/tiktok-rules/calculate-roas', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          tiktokAccountId: formData.tiktokAccountId,
  ...
  const handleSimulate = async () => {
    if (!formData.tiktokAccountId) {
      setError('Selecciona una cuenta TikTok para simular');
      return;
    }
    ...
    try {
      const response = await fetch('/api/tiktok-rules/simulate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: formData.name || 'Nueva Regla TikTok',
          tiktokAccountId: formData.tiktokAccountId,
          level: formData.level,
          applyToAllCampaigns: formData.applyToAllCampaigns,
          ...
          scheduleDays: formData.scheduleDays,
          // ROAS specific
          tonicAccountId: formData.metric === 'ROAS' ? formData.tonicAccountId : undefined,
          roasDateRange: formData.metric === 'ROAS' ? formData.roasDateRange : undefined,
        }),
      });
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            {loadingAccounts ? (
              ...
              <div className="w-full px-4 py-2 border border-red-200 rounded-lg bg-red-50 text-red-600">
                No hay cuentas TikTok configuradas
              </div>
            ) : (
              <select
                value={formData.tiktokAccountId}
                onChange={e => setFormData({ ...formData, tiktokAccountId: e.target.value, specificCampaignIds: [] })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-900 focus:border-gray-900"
              >
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <div className="space-y-4">
          ...
          {/* Campaign Selector */}
          {!formData.applyToAllCampaigns && formData.tiktokAccountId && (
            <div className="mt-4">
              ...
              {loadingCampaigns ? (
                ...
                <div className="w-full px-4 py-2 border border-yellow-200 rounded-lg bg-yellow-50 text-yellow-700">
                  No hay campanas activas con esta cuenta TikTok
                </div>
              ) : (
              ...
              <p className="text-xs text-gray-500 mt-1">
                Solo se muestran campanas ACTIVAS que usan esta cuenta TikTok.
              </p>
            </div>
          )}
        </div>
          ...
          {formData.metric === 'ROAS' && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-4">
              <div className="flex items-start gap-2 mb-2">
                ...
                <div>
                  <h4 className="font-medium text-gray-900">Calculo de ROAS Hibrido</h4>
                  <p className="text-xs text-gray-700">
                    El ROAS se calculara usando: (Gross Revenue de Tonic / Costo de TikTok) x 100
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Periodo de tiempo para obtener datos de Tonic y TikTok
                </p>
              </div>
              ...
              <div className="pt-2">
                <button
                  ...
                  disabled={calculatingRoas || !formData.tiktokAccountId || !formData.tonicAccountId}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={simulating || !formData.tiktokAccountId || (!formData.applyToAllCampaigns && formData.specificCampaignIds.length === 0)}
          ...
          <button
            type="submit"
            disabled={loading || tiktokAccounts.length === 0}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-gray-800 to-gray-900 px-6 py-4 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-white">Simulacion de Regla TikTok</h3>
                <p className="text-gray-300 text-sm">{simulationResult.rule.name}</p>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
                  <div>
                    ...
                    <span className="ml-2 font-semibold text-gray-900">{simulationResult.rule.level}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium">Metrica:</span>
                    <span className="ml-2 font-semibold text-gray-900">{simulationResult.rule.metric}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium">Condicion:</span>
                    <span className="ml-2 font-semibold text-gray-900">{simulationResult.rule.operator} {simulationResult.rule.value}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium">Accion:</span>
                    <span className="ml-2 font-semibold text-gray-900">{simulationResult.rule.action}</span>
                  </div>
                  <div>
                    <span className="text-gray-700 font-medium">Frecuencia:</span>
                    <span className="ml-2 font-semibold text-gray-900">Cada {simulationResult.rule.frequencyHours}h</span>
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-gray-100 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-gray-900">{simulationResult.summary.totalEntities}</div>
                  <div className="text-sm text-gray-700">Entidades totales</div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4 text-center">
                  <div className="text-3xl font-bold text-gray-600">{simulationResult.summary.entitiesWithData}</div>
                  <div className="text-sm text-gray-600">Con datos</div>
                </div>
                ...
                <div className="bg-gray-900 rounded-lg p-4 text-center">
                  <div className="text-2xl font-bold text-white">{simulationResult.summary.actionThatWouldExecute}</div>
                  <div className="text-sm text-gray-300">Accion</div>
                </div>
              </div>
      ...
      {showRoasModal && roasResult && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-gray-800 to-gray-900 px-6 py-4 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-white">Calculo de ROAS - TikTok</h3>
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                ...
                <div className="bg-gray-50 rounded-lg p-4 text-center">
                  ...
                  <div className="text-sm text-gray-600">Campanas Evaluadas</div>
                </div>
              </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-gray-200">
                        <th className="text-left px-3 py-2 rounded-tl-lg font-semibold text-gray-900">Campana TikTok</th>
                    ...
                    <tbody>
                      {roasResult.campaigns.map((campaign, idx) => (
                        <tr key={campaign.tiktokCampaignId} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                          <td className="px-3 py-2 font-medium text-gray-900 max-w-[200px] truncate" title={campaign.tiktokCampaignName}>
                            {campaign.tiktokCampaignName}
                          </td>
              ...
              <div className="mt-6 bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-xs text-gray-500 mt-3">
                  El mapeo de campanas se realiza extrayendo el ID de Tonic del nombre de la campana en TikTok (formato: &quot;1234567_NombreCampana&quot;)
                </p>
              </div>
            </div>
