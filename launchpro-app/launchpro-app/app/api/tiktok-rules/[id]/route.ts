...
/**
 * GET /api/tiktok-rules/[id]
 * Get a single TikTok rule with its execution history
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    logger.info('api', `GET /api/tiktok-rules/${id}`);
    ...
    logger.success('api', `Fetched TikTok rule: ${rule.name}`, { ruleId: id }, duration);
    ...
    logger.error('api', `Error fetching TikTok rule: ${error.message}`, {
      ruleId: id,
      stack: error.stack,
    });
...
/**
 * PUT /api/tiktok-rules/[id]
 * Update an existing TikTok rule
 */
export async function PUT(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    logger.info('api', `PUT /api/tiktok-rules/${id}`, { updates: Object.keys(body) });

    // Verify rule exists and is a TikTok rule
    ...
    // Validate level (TikTok uses AD_GROUP instead of AD_SET)
    if (body.level) {
    ...
    if (body.tiktokAccountId !== undefined) updateData.tiktokAccountId = body.tiktokAccountId;
    ...
    const duration = Date.now() - startTime;
    logger.success('api', `TikTok rule updated: ${rule.name}`, { ruleId: id }, duration);
    ...
    logger.error('api', `Error updating TikTok rule: ${error.message}`, {
      ruleId: id,
      stack: error.stack,
    });
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    logger.info('api', `DELETE /api/tiktok-rules/${id}`);
    ...
    const duration = Date.now() - startTime;
    logger.success('api', `TikTok rule deleted: ${existingRule.name}`, { ruleId: id }, duration);

    return NextResponse.json({
      success: true,
      message: 'TikTok rule deleted successfully',
    });
  } catch (error: any) {
    logger.error('api', `Error deleting TikTok rule: ${error.message}`, {
      ruleId: id,
      stack: error.stack,
    });
